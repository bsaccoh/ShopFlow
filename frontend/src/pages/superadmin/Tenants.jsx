...
const Tenants = () => {
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            const res = await adminApi.resetTenantPassword(resetPasswordTenant.id, newPassword);
            if (res.success) {
            ...
            render: (row) => {
                ...
                return (
                    <div className="flex justify-end gap-1.5">
                        ...
                        <button onClick={() => { setResetPasswordTenant(row); setNewPassword(''); }}
                            className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-md transition-colors" title="Reset Password">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
                        <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
                            ...
                            <button onClick={() => setResetPasswordTenant(null)} className="p-1.5 hover:bg-slate-100 rounded-full">
                                <X className="w-4 h-4 text-slate-400" />
                            </button>
                        </div>
                        <form onSubmit={handleResetPassword} className="p-5 space-y-4">
                            <p className="text-sm text-slate-600">
                                Set a new password for <span className="font-semibold">{resetPasswordTenant.name}</span>'s admin account.
                            </p>
                            <div>
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                <button type="button" onClick={() => setResetPasswordTenant(null)} className="btn-secondary px-4 py-2 text-sm">Cancel</button>
                                <button type="submit" disabled={resettingPassword} className="btn-primary px-4 py-2 text-sm">
